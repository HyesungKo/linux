...
When I compared the total number of exit of VM with EPT and VM without EPT, VM without EPT exited almost 10 times more than VM with EPT. In terms of the comparison of cycles of total exits, VM without EPT took almost 7 times more cycle than VM with EPT. I expected the result above because VM without EPT (shadow paging) was supposed to perform more vm-exits than VM with EPT (nested paging). In addition, #PF exception (0) was expected to perform more without ept and the exits related to EPT, EPT violation and EPT misconfiguration, were performed only on ept as expected. 
